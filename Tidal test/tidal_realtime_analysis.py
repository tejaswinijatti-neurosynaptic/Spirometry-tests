...
async def RelealTimeAnalysis(message):
    ...
    if _encoder_task is None:
        ...
        threading.Timer(30.0, drain_and_use).start()
...
async def encoder_loop():
    """
    Pop one frame (20 samples), FIR it (streaming), encode, send.
    Never grows payload size.
    """
    while True:
        try:
            try:
                frame = frame_queue.get_nowait()  # list[20]
            except queue.Empty:
                await asyncio.sleep(ENCODE_EVERY_SEC)
                continue

            if _SEND_ASYNC is None:
                # sender not injected yet
                await asyncio.sleep(ENCODE_EVERY_SEC)
                continue

            # 1) Streaming FIR
            yf = fir_process_frame(frame, W_TRI, _fir_buf)

            # 2) Choose what to send
            if yf:
                y = np.asarray(yf, dtype=float)
            else:
                # FIR warm-up period: optionally send raw
                if not SEND_RAW_UNTIL_FIR_READY:
                    await asyncio.sleep(ENCODE_EVERY_SEC)
                    continue
                y = np.asarray(frame, dtype=float)

            # 3) Build x for just this packet (local timebase 0..n*DT)
            x = np.arange(y.size, dtype=float) * DT

            # 4) Encode and send
            encoded = prepare_spiro_data(x, y, normalize=True)
            payload = json.dumps(encoded, separators=(',', ':'))
            await _SEND_ASYNC("dataFromPython~" + payload)
...
async def processData(message):
    with open(LOG_FILE, "a", buffering=1, encoding="utf-8") as f:
        ...
        if message:
            ...
            print("decoded samples:", len(data))
            if data:
...
def queue_to_ndarray_drain() -> np.ndarray:
    items = []
    while True:
        try:
            items.append(pressure_queue.get_nowait())
        except queue.Empty:
            break
    return np.array(items)

def drain_and_use():
    arr = queue_to_ndarray_drain()
    TV_calculations.calculateFinalTV(arr)
