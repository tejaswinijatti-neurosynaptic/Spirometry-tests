...
async def ws_listener():
    global ws_conn, client_should_stop, DEBUGGING
    DEBUGGING = True
    try:
...
def update(_):
    ...
    if csv_rows and (not DEBUGGING) and _SEND_ASYNC is not None:
        ...
        # convert from list-of-rows -> arrays
        t  = np.array([float(r[0]) for r in csv_rows], dtype=float)
        p  = np.array([float(r[1]) for r in csv_rows], dtype=float)

        # FV values may be "" when inactive; filter those out
        fv_pairs = [(r[3], r[2]) for r in csv_rows if r[2] != "" and r[3] != ""]
        if fv_pairs:
            v_fv = np.array([float(v) for (v, f) in fv_pairs], dtype=float)
            f_fv = np.array([float(f) for (v, f) in fv_pairs], dtype=float)
        else:
            v_fv = np.array([], dtype=float)
            f_fv = np.array([], dtype=float)
    ...
    if exhale_timer_running and exhale_start_walltime is not None:
        ...
        if not long_beep_played:
            ...
            if _SEND_ASYNC is not None:
                ...
                try:
                    loop = asyncio.get_event_loop()
                    loop.create_task(_SEND_ASYNC(f"dataFromLib~107~event~{payload}"))
                except:
                    pass  # silently fail if no event loop
    else:
